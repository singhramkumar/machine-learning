...
class Sigmoid:
    ...
    def activate(self, values):
        ...
        result = sigmoidLogistic(strength)
        print("======")
        print(result)
        return result
    ...
    def update(self, values, train, eta=.1):
        ...
        for X, y_true in zip(values, train):
            ...
            deriv = sigmoidLogiscticDerivation(self.last_input)
            print(deriv)
            # TODO: update self.weights based on learning rate, signal accuracy,
            # function slope (derivative) and input value
            delta = eta*(deriv*X*(y_true - y_pred))
            print(delta)
            self.weights = self.weights + delta
            print(self.weights)
            print('-------------')
            
def sigmoidLogistic(x):
    return (1/(1+np.exp(-x)))
    

def sigmoidLogiscticDerivation(x):
    return (sigmoidLogistic(x)*(1-sigmoidLogistic(x)))
